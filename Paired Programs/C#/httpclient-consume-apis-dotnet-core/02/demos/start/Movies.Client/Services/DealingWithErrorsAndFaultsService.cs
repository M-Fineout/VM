            ...
            CancellationToken cancellationToken)
        {
                ...
                "api/movies"))
            {
                        ...
                        cancellationToken))
                {
                    if (!response.IsSuccessStatusCode)
                    {
                        if (response.StatusCode == System.Net.HttpStatusCode.UnprocessableEntity) //422 --validation error
                        {
                            //we can't just access the content. Remember that we're sending this request with the HttpCompletionMode set to ResponseHeadersRead, so the content will still come streaming in at the moment we try to read it out. If we don't use a stream for that, this will fail. To fix this, we call ReadAsStreamAsync on the content, and on that errorStream we call ReadAndDeserializeFromJson.
                            var errorStream = await response.Content.ReadAsStreamAsync();
                            var validationErrors = errorStream.ReadAndDeserializeFromJson();
                            Console.WriteLine(validationErrors); //returns validationError when the attempt to deserialize fails
                            return;
                        }
                        else
                        {
